...
import styles from './MandalDetails.css';
...
const MandalDetails = () => {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            ...
            console.log(user.id);
            try {
                const response = await axios.post(`${BACKEND_ENDPOINT}/user/yuvak_by_mandal`, {
                    shibir_id: user.id
                });
            ...
            <table>
                ...
                <tbody>
                    {Array.isArray(details) && details.map((yuvak, index) => (
                        <tr>
                            <td>{yuvak.name}</td>
                            <td style={{ paddingLeft: '1.8rem' }}>{yuvak.phone_number}</td>
                            <td>
