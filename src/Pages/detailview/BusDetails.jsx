...
import styles from './BusDetails.css';
...
const BusDetails = () => {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            ...
            console.log(user.id);
            try {
                const response = await axios.post(`${BACKEND_ENDPOINT}/admin_panel/detail_for_bus_leader`, {
                    shibir_id: user.id
                });
                setDetails(response.data.yuvaks);
            } catch (error) {
                console.error(error);
            }
        }
            ...
            <table>
                ...
                <tbody>
                    {Array.isArray(details) && details.map((yuvak, index) => (
                        <tr>
                            <td>{yuvak.name}</td>
                            <td style={{ paddingLeft: '1.8rem' }}>{yuvak.seat_no}</td>
                            <td>
                    ...
                    <DetailedViewModal>
                        ...
                        {singleData && (
                            <DetailedViewData>
                                ...
                                <p><strong>Seat:- </strong>{singleData.seat_no}</p>
                                <p><strong>Xetra(Mandal):-</strong> {singleData.xetra_mandal}</p>
